...
var correctGuess = 0;
var wrongWord = 1;
var rand = Math.floor(Math.random() * 15761);
var word="wolves";
...
reset.addEventListener('click',(e)=>{
   ...
   if(resetkeypressed!='Enter'){
    rand = Math.floor(Math.random() * 15761);
    fetcher()
    addloader();
    removeloader();
    for(var i=0;i<42;){
...
setTimeout(() => {
    ...
    async function checkForWordInList(Word) {
        const response = await fetch("words6.json");
        const json = await response.json();
        for (var i = 1; i < 15761; i++) {
            if (json[i] === Word.toLowerCase()) {

                wrongWord = 0;

                if (correctGuess != 6 && inputWordLength === 6) {
                    ...
                    if(inputContainer===36 && correctGuess!=6){
                    ...
                    inputWordLength = 0;
                    inputContainer = inputContainer + 6;
                }
                 
                if (correctGuess === 6 && flag===0) {
...
function switcher(value){
    ...
    switch(Number(value)){
        ...
        break;
        case 3: window.location.assign('/index.html');
        break;
    }
}